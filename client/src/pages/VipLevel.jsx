...
const VipLevel = () => {
  ...
  const currentPoints = summary?.totalCredit || 0;

  // Dynamic points and progress
  const levels = levelData.map(level => ({
    ...level,
    amount: Number(level.amount.replace(/,/g, '')) // Remove commas and convert to number
  })).sort((a, b) => a.amount - b.amount);

  // Find the current level and the next level's required points
  let currentLevelIndex = 0;
  for (let i = 0; i < levels.length; i++) {
    if (currentPoints >= levels[i].amount) {
      currentLevelIndex = i;
    }
  }

  const requiredPoints = levels[currentLevelIndex + 1]?.amount || levels[currentLevelIndex].amount; // Use next level or keep last level
  const progressPercentage = Math.min((currentPoints / requiredPoints) * 100, 100);
  const pointsLeft = Math.max(requiredPoints - currentPoints, 0);
      ...
      <div className="p-[0_6.266667vw]">
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-[#949494] text-[3.733333vw] mt-[1.333333vw]">
              +91 790****205
            </p>
          </div>
        </div>
        ...
        <div
          className="w-full min-h-[40vw] mb-[0.666667vw] mt-[6.666667vw] relative"
          style={{
        ...
        >
          {/* Current Level Label */}
          <div
            className="absolute flex justify-center items-center top-0 right-0 rounded-[0_1.333333vw_0_0] h-[5.866667vw] text-white text-[2.933333vw] p-[0_1.333333vw]"
            style={{
              background:
                "linear-gradient(270deg, rgb(0, 0, 0), rgba(0, 0, 0, 0.8) 100%)",
            }}
          >
            Current Level
          </div>
          ...
          <div
            className="bg-clip-text text-transparent text-[10.666667vw] font-bold pl-[18%] pt-[4%]"
            style={{
              backgroundImage:
                "linear-gradient(1.97deg, rgb(56, 30, 6), rgb(143, 98, 37))",
            }}
          >
            Lv.{level}
          </div>
          ...
          <div className="text-[2.933333vw] ml-[5.333333vw] mt-[2.666667vw]">
            {pointsLeft} left to upgrade to next level
          </div>
          <div
            className="h-[2px] m-[6vw_4.666667vw_3.733333vw] relative rounded-[1.066667vw]"
            style={{ background: "rgb(247, 242, 236)" }}
          >
            <span
          ...
          <div className="flex items-center justify-between pb-[20px]">
            <div className="flex items-center">
              ...
              <div className="text-[3.2vw] mt-[1px] mr-[0.666667vw]">
                {currentPoints}/{requiredPoints}
              </div>
            </div>
            <div className="text-[2.4vw] mr-[4.666667vw]">
              Unlock 1 additional benefit after upgrading
            </div>
          </div>
        </div>

        {/* Membership Level Section */}
        <p className="text-[5.333333vw] text-black mb-[2vw] mt-[5.333333vw]">
          Membership Level
        </p>
        <LevelItem />
      </div>
